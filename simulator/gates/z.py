...
class Z(Gate):
    ...
    def apply(self, state_vector):
        """
        Apply the Z gate to a quantum state vector.
        ...
        if self.qubits is None or len(self.qubits) != 1:
            raise ValueError("Z gate requires exactly one target qubit.")
        ...
        # Construct the full operator by combining identity and Z gate
